...
`game rect` remains a read-only query — and it is the read for layout output: on a
`Control`, `game get --property position` / `size` / `global_position` / `global_rect`
all refuse with `live_unknown_property` and name it.
