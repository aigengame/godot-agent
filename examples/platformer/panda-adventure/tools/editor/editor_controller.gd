...
##
## A separate development entry scene (tools/editor/editor.tscn) in the same
## Godot project. The game's default UI composition root remains untouched. It
## loads Level 1's JSON
...
## instancing the game's own UI composition root against the freshly derived
...
const EditorLevelModelScript := preload("res://tools/editor/editor_level_model.gd")
const EditorBuilderScript := preload("res://tools/editor/editor_builder.gd")
const EditorFormsScript := preload("res://tools/editor/editor_forms.gd")
...
const ScaleSpecConfigScript := preload("res://content/config/scale_spec_config.gd")
const MainScene := preload("res://ui/game_shell.tscn")

const GENERATED_DIR := "res://content/data/generated"
const SCALE_SPEC_PATH := "res://content/data/generated/scale_spec.tres"
...
## read from `content/data/schema/*.json` (never a hand-forked registry). A form edit
...
## convention (content/data/schema/<name>.schema.json).
func _section(authority: String, schema_name: String, title: String) -> Dictionary:
	return {
		"authority": authority,
		"schema_path": "res://content/data/schema/%s.schema.json" % schema_name,
		"title": title,
	}
...
## The running Game Shell instance the palette reaches through its `_editor`
## reference, or null in edit mode.
...
func _enter_play() -> void:
	# Play the edited level: persist + re-derive any pending edits so the Game
	# Shell's Gameplay child loads fresh Resources, then refresh the process resource
