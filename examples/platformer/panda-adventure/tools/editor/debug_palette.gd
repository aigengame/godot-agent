...
##
## The palette node lives in the development Editor scene, so the export
## exclude_filter (`tools/editor/*`) strips it from player builds with the rest
## of the editor (gADR-0012). It acts on the play instance the EditorController
## hosts (the Game Shell under PlayHost) — a debug tool's tight coupling to runtime
...
const EnemyControllerScript := preload("res://content/controllers/enemy_controller.gd")
...
## Reaches the Player through the Gameplay controller's explicit public entry.
...
func _do_spawn() -> void:
	...
	enemy.setup(kind, level.player_node())
...
## The running Gameplay controller inside the hosted Game Shell, or null when not
## playing.
func _play_root() -> Node:
	if not _is_playing():
		return null
	var shell: Node = _editor.get_play_instance()
	return shell.get_node_or_null("Gameplay") if shell != null else null


func _find_player() -> Node:
	var level := _play_root()
	return level.player_node() if level != null else null
