...
Where the *Scale Spec* owns size, this chapter owns *look* — the qualitative art direction the
produced assets carry. Per gADR-0000 it is the same two-layer split applied to appearance: this
chapter fixes the intent in prose, while the machine-consumable style parameters (the shared
style keywords and prompt fragment, the bounded pixel-art palette, and the per-category style
hints) are **data** — the asset pipeline's *Style descriptor*, a pipeline-only config, not the
JSON authority and never derived to a Resource (the game reads assets, not their style;
gADR-0014). No color or keyword is fixed as a value here on purpose.

**Direction.** A stylized **space-opera frontier** on the edge of a black hole, read through a
**Great-Wall rampart** motif, blending **fantasy with realism** — a fantastical premise rendered
with grounded, readable forms. The direction is one look shared by every asset regardless of how
it was acquired (downloaded or generated), so the world reads as a whole rather than a collage.

**Regime.** All look lives inside the pixel-art regime the *Scale Spec* establishes (one art
pixel to one world pixel). Two qualitative rules make mixed-source assets cohere without a human
art director in the loop:

- **Bounded palette.** Every asset is quantized to one shared, bounded palette, so a downloaded
  crate and a generated prop sit in the same color world. The palette leans earthy and grounded
  (wood, stone, metal for the rampart and its props) with a few saturated space-opera accents;
  the exact entries are Style-descriptor data.
- **Readability outranks realism.** Silhouettes stay clean and legible at gameplay scale; detail
  never competes with the *Scale Spec*'s Tier-and-actor readability. The fantasy-realism blend is
  carried by form and material, not by rendering fidelity.

**Provenance & licensing.** Produced assets are CC0/CC-BY only, recorded with their provenance and
license in the *Asset manifest* (a pipeline invariant, gADR-0014) — the art direction is realized
within that sourcing constraint, never against it.

> **Scoped rule (2026-07-10, P2-S9/#445 — user-approved):** the CC0/CC-BY rule above holds for every
> image category; **fonts additionally may be OFL** (the SIL Open Font License — a permissive *font*
> license), the one category-scoped exception, recorded and gated per category (gADR-0014 amendment).
> The HUD font is *Press Start 2P* (OFL). Every non-font category stays CC0/CC-BY.
