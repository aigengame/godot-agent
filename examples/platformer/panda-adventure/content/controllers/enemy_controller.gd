...
const EnemyWarpDriverScript := preload("res://content/controllers/enemy_warp_driver.gd")
const EnemyProjectileScene := preload("res://content/scenes/enemy_projectile.tscn")
...
## Hand this enemy its kind and explicit target before it enters the tree.
func setup(kind: EnemyConfigScript, target: Node2D) -> void:
	_kind = kind
	_target = target
...
func _physics_process(delta: float) -> void:
	...
	var move_dir := 0.0
	if player != null:
		move_dir = EnemyAIScript.compute_move_dir(
			position,
			player.position,
			_kind.aggro_range,
			_kind.keep_range_min,
			_kind.keep_range_max,
		)
...
## The target injected by the Content spawner.
func _player() -> Node2D:
