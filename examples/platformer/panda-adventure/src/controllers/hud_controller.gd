...
func _process(_delta: float) -> void:
	if _config == null:
		return
	var player := _player()
	if player == null or not player.has_method("hud_state"):
		return
	var state: Dictionary = player.hud_state()
	if state.is_empty():
...
## in _ready) — no cached reference to go stale, and boot order stays
## irrelevant (an empty lookup just skips the frame).
func _player() -> Node:
