...
## machine headless. Content orchestrates: it feeds the two upstream edges,
## applies the World freeze, publishes the result to UI, and serves Retry through
## an explicit application entry point. The UI-owned Game Shell shows the End screen.
...
## - "changed": this event entered an End state — the caller freezes the world,
##   logs the verdict, and publishes the result exactly once.
