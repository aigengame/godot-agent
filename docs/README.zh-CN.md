...
| `daemon uninstall` | 从项目中移除游戏内 `gda` harness——autoload 条目、harness 文件及其 `.uid` 附属文件，以及不再包含任何键的 `[autoload]` 段，使 `project.godot` 恢复到安装前的字节内容；结果会逐项列出被移除的每个路径与段。这是一次显式的开发工具卸载；`gda export run` 在导出产物时已经会自动剥离 harness。 |
