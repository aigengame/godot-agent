...
| `harness_install_permission_denied` | `environment` | `classifier` | `127` | The gda harness install (`gda daemon start` / `gda daemon install`) could not access the project's filesystem — a filesystem-restricted sandbox denied a read or a write the install needed, and the message names the failing path — so the install was refused; any partial write is rolled back where the filesystem still allows it (Phase 2, #700). |
