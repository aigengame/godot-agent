...
| `harness_install_unwritable` | `environment` | `classifier` | `127` | The gda harness install (`gda daemon start` / `gda daemon install`) could not write to the project's filesystem — a filesystem-restricted sandbox refused the write, and the message names the failing path — so the install was refused; the partial write is rolled back where the filesystem still allows it (Phase 2, #700). |
