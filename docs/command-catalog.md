...
file). `skipped` qualifies that list the way it qualifies `export run`'s (#990): `created`
is complete when the count is `0`, and the field's own description is the rule. An `invalid` or `failed` asset also says WHY (#853): `reason` names the check that
...
list is the authoritative inventory, complete when its `skipped` is `0`). This prediction is a SECOND spelling of the walk's
