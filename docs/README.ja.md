...
何も指定されないままコンパイルされることはありません。入れ子の場合、メッセージは再実行の
形をすべて示します。渡すべき `--project` と、その所有者からの相対で綴り直した対象です。
相対パスはプロジェクトを基準に解決されるため、元の綴りは新しいプロジェクトでは見つかりません。
`script run` と `resource import` も
同じコードで同様に拒否します。`--all` にはこの拒否は不要です。プロジェクト全体を走査する
ウォークが、エンジン自身のスキャンと同じく、入れ子の `project.godot` を持つディレクトリ
(および `.gdignore` を持つディレクトリ)をスキップするため、名指しなら拒否されるファイルを
そもそも列挙しません。
