...
| コマンド | 機能 |
| ------- | ------------ |
| `asset-pipeline run` | 選択した PNG/GLB ファイルをステージし、必要に応じて PNG をリサイズしてからインストールとインポートを行い、Godot が実際にロードした型と寸法またはシーンを確認します。 |

ソースからターゲットへのマッピングと上書きポリシーを明示してください。生成が完了した画像ファイルも
同じ手順で処理でき、呼び出し側が指定したメタデータを任意で付加できます。[ファイル引き渡しガイド](../libs/gda-assets/README.md)では、
入力、参照、部分的な失敗を扱います。このワークフローは gda に同梱されているため、別のアセットツールは不要です。
