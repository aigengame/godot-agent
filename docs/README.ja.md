...
- **🛡️ 静かに失敗せず、はっきり失敗する。** 見つからない、あるいはハングしたエンジンはタイムアウト
  ...
  分岐できます。`gda` が認識しないコマンドやオプションも同じ構造化された形で拒否され、代わりに
  使うべき呼び出しが `hint` に入ります。
...
解決されたプロジェクトの**外**にあるパスは、その偽のエラー群を報告する代わりに、ファイルと
...
| `game call` | ノードのクラスが呼び出し可能と宣言した(`GDA_CALLABLE`)メソッドを 1 つ呼び出し、その戻り値を投影します。 |

`game call` は `game get` では読めないもの——プロジェクトが**メソッド**として公開する
デバッグ/状態コントラクト——を読みます。クラスは自身のソース内の `GDA_CALLABLE` スクリプト
定数で呼び出し可能なメソッドを宣言し、gda はその定数を静的に読み取ります。したがって「何を
呼び出してよいか」を知る段階でプロジェクトのコードは一切実行されず、宣言するまでどのメソッドも
呼び出せません。この定数は継承チェーン全体の宣言です。GDScript は基底クラスの定数の再宣言を
禁じるため、1 つのチェーンにつき宣言するクラスはちょうど 1 つです(基底クラスが宣言する場合、
そのサブクラスの呼び出し可能面も基底クラスが持ちます)。gda は宣言されたメソッドに副作用が
...
- **`gda game call` は実行中のゲームで宣言済みメソッドを 1 つ実行します。** 1 リクエストにつき、
  対象ノードのクラスが `GDA_CALLABLE` スクリプト定数で宣言したそのメソッドだけが実行されます。
