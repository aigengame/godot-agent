...
| `resource inspect-model-content` | 実行中のインスタンスとの比較用に、インポート済み GLB のサポート対象となる静的コンテンツを範囲を限定してダイジェスト化します。 |
...
### アセットワークフロー — Godot 4.4 以上、全プラットフォーム

| コマンド | 機能 |
| ------- | ------------ |
| `asset-pipeline run` | 保存済みの Blender ファイルから選択したサブツリーをエクスポートするか、PNG/GLB ファイルを準備し、配置・インポート後に Godot の実際の読み込み結果を確認します。対象ファイルとインポート結果のハッシュ収集や、テストシーンの再起動後に実行中の GLB インスタンスを比較する処理も選択できます。 |
...
Blender からのエクスポート、ファイル入力、参照、ランタイム更新、隔離プレビュー、エクスポート済みパッケージの検証、部分的な失敗を扱います。更新時にはランタイム状態が破棄され、
シーンとインスタンスのパスを明示する必要があります。[静的モデルコンテンツガイド](model-content.md)では、基礎となる 2 つの
ファクトコマンドと共通の測定方式を定義しています。このワークフローは gda に同梱されているため、別のアセットツールは不要です。
...
| `game inspect-model-content` | 選択したランタイムモデルインスタンス以下のサポート対象となる静的コンテンツを範囲を限定してダイジェスト化します。 |
...
- **シーンスクリプトの `_init`** は、シーンをインスタンス化する操作では必ず実行されます。すべての変更系
  `node` コマンド、`node get`、`resource inspect-model`、`resource inspect-model-content`、`asset-pipeline check --path`、
