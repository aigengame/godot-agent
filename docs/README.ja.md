...
示す `scene` を持ち、その `path` とノードはそのファイルに対して読みます。境界は参照された
シーン**ファイル**であって、行が宣言する型ではありません。Godot はシーンを読み込むときすべての
`[ext_resource]` を読み込み、ハンドラを拡張子で選ぶため、単なるメタデータとして参照された
`.tscn` もインスタンス化されたものとまったく同じように親を壊します。走査は 3 種類の辺をたどらずに
報告します。`cyclic_instance`——参照が閉じる循環で、エンジンはその閉じる辺を破棄し、それが与える
はずだったノードも消えます；`unreadable_sub_scene`——走査が読むシーンテキストを持たないバイナリ
の `.scn`；そして `instance_depth_exceeded`——サブシーン 16 段以内のどの経路からも到達しない
シーンです。後の 2 つはそのサブツリーが健全なのではなく**未検査**であることを示すので、結論が
必要ならそのシーンを直接検証するか、`.tscn` として保存し直してください。この深さの上限が抑えるのは
