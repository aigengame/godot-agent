...
- **🛡️ 静かに失敗せず、はっきり失敗する。** 見つからない、あるいはハングしたエンジンはタイムアウト
  で境界が区切られ、**安定した非ゼロの終了コード** と、失敗のカテゴリ・コード・型付きの証拠を運ぶ
  Error エンベロープにマッピングされます。これにより、シェルやエージェントは文章を解析せず失敗で
  分岐できます。`gda` が認識しないコマンドも同じ形で拒否され、代わりに使うべき呼び出しが `hint` に入ります。
...
| `scene validate` | シーンを**静的に**検査します——依存が解決でき、バインドされたスクリプトがコンパイルできるかを、サブシーンも含めて、何も実行せずに調べます。壊れたシーンはエラーではなく判定(`valid: false`、終了コード `0`)です。 |
...
| `script run` | プロジェクトスクリプトをワンショットのエントリポイントとして headless 実行します。上限は `--timeout` です。スクリプトの `exit_status`・`stdout`・`stderr` はそのまま通されます——非ゼロの `quit()` は失敗ではなくデータですが、`--strict` を渡せば失敗になります。 |
...
| `game call` | ノードのスクリプトが `GDA_CALLABLE` で宣言したメソッドを 1 つ呼び出し、その戻り値を投影します——宣言されていないものは決して呼び出されません。 |
...
| `--version` | インストール済みの `gda` のバージョンを表示します。`--json` を付けると、その出どころも出力します — インストール種別(`wheel` または `editable`)と、editable インストールの場合はソースチェックアウトの Git リビジョンです。 |
...
ドメインコマンドは、`res://` パスが解決できるように **Godot プロジェクト** を解決します。明示的に指定した
ディレクトリはプロジェクトでなければならず、そうでなければ `gda` はエラーを報告します。どれも解決されない場合、
`gda` は **projectless(プロジェクトなし)** で動作します — 通常のファイルシステムパスは使えますが、`res://` は使えません。
