...
- model-source.json — distinct player/enemy Symbols, two pure Formulas, two directional cast
  entrypoints, and one explicit cancellation wrapper entrypoint;
- experiment.json — one reciprocal scenario, two same-time roots, exact Model bindings, seed,
  six dimensioned Metrics, and acceptance policy.
...
## 11. Product and architecture review

The human owner should run the baseline, cancellation, eligibility and tuning variants, then
