...
def _rename_source_semantics(model_source: dict[str, Any]) -> None:
    ...
    for entrypoint in model_source["entrypoints"]:
        entrypoint["id"] = _renamed(entrypoint["id"])
    ...
    for scenario in experiment["scenarios"]:
        for event in scenario["event_plan"]:
            if "entrypoint" in event:
                event["entrypoint"] = _renamed(event["entrypoint"])
...
def _rename_selected_authority(language, kernel) -> None:
    vector_sets = {
...
def _selected_dependency_binding(
...
def build_priority_case(*, renamed: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    """Build one authored priority case and its selected-dependency binding."""
    kernel, language, operations = authorities()
    if renamed:
        _rename_selected_authority(language, kernel)
    ...
    dependency_binding = _selected_dependency_binding(artifacts)
    ...
    proof = {
        ...
        "selected_dependency_binding": dependency_binding,
