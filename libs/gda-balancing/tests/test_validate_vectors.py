...
Drives the command end to end through ``run_legacy_cli`` (in-process dispatch, real
...
def _run(run_legacy_cli, tmp_path, content, name="doc.json"):
    return run_legacy_cli(["design", "validate", _doc(tmp_path, content, name)])
...
def test_v1_minimal_document_validates(run_legacy_cli, minimal_design_path):
...
def test_v12_patch_is_ignored_for_acceptance(run_legacy_cli, tmp_path):
    ...
    exit_code, stdout, stderr = _run(run_legacy_cli, tmp_path, content)
    assert (exit_code, stderr) == (0, "")
    assert stdout == canonical_json({"valid": True})
...
def test_v12_newer_minor_is_unsupported(run_legacy_cli, tmp_path):
    content = '{"schema_version": "1.7.0", "meta": {"name": "smallest"}}'
    exit_code, stdout, _ = _run(run_legacy_cli, tmp_path, content)
    assert exit_code == 2
...
def test_non_semver_version_is_malformed(run_legacy_cli, tmp_path):
...
def test_missing_version_is_malformed(run_legacy_cli, tmp_path):
...
def test_unparseable_text_is_malformed_json(run_legacy_cli, tmp_path):
    exit_code, stdout, _ = _run(run_legacy_cli, tmp_path, '{"schema_version": "1.0.0"')
    assert exit_code == 2
    refusals = _refusals(stdout)
...
def test_duplicate_object_key_is_refused(run_legacy_cli, tmp_path):
...
def test_duplicate_keys_at_distinct_paths_do_not_collapse(run_legacy_cli, tmp_path):
...
def test_deep_nesting_is_a_refusal_not_a_crash(run_legacy_cli, tmp_path):
...
def test_oversized_collection_is_refused(run_legacy_cli, tmp_path):
    ...
    exit_code, stdout, _ = _run(run_legacy_cli, tmp_path, json.dumps(document))
    assert exit_code == 2
    codes = {r["code"] for r in _refusals(stdout)}
    assert "collection_too_large" in codes


def test_non_finite_number_is_refused_at_its_path(run_legacy_cli, tmp_path):
...
def test_huge_integer_is_number_not_finite_not_a_crash(run_legacy_cli, tmp_path):
...
def test_precision_losing_integer_is_accepted(run_legacy_cli, tmp_path):
...
def test_digit_limit_integer_is_malformed_json_not_a_crash(run_legacy_cli, tmp_path):
...
def test_oversized_document_is_refused(run_legacy_cli, tmp_path):
...
def test_report_all_is_ordered_by_path(run_legacy_cli, tmp_path):
...
def test_surrogate_string_value_is_string_not_unicode(run_legacy_cli, tmp_path):
...
def test_surrogate_object_key_is_string_not_unicode(run_legacy_cli, tmp_path):
...
def test_duplicated_surrogate_key_is_refused_at_a_safe_path(run_legacy_cli, tmp_path):
...
def test_surrogate_key_deep_subtree_reports_no_surrogate_path(run_legacy_cli, tmp_path):
...
def test_sibling_surrogate_keys_aggregate_into_one_named_refusal(
    run_legacy_cli, tmp_path
):
...
def test_duplicated_sibling_surrogate_keys_lose_nothing(run_legacy_cli, tmp_path):
...
def test_surrogate_keys_under_different_ancestors_stay_separate(
    run_legacy_cli, tmp_path
):
...
def test_surrogate_document_never_reaches_format_emission(run_legacy_cli, tmp_path):
    ...
    exit_code, stdout, stderr = run_legacy_cli(["design", "format", path])
    assert exit_code == 2  # specifically NOT 4
    assert _is_utf8_encodable(stdout)
...
def test_trailing_newline_schema_version_is_malformed(run_legacy_cli, tmp_path):
...
def test_v1_unknown_top_level_key_refuses_at_the_element(run_legacy_cli, tmp_path):
...
def test_type_violation_reports_the_offending_scalar_path(run_legacy_cli, tmp_path):
...
def test_wrong_typed_attribute_entry_refuses_within_its_element(
    run_legacy_cli, tmp_path
):
...
def test_v2_untyped_formula_ref_is_a_structural_refusal(run_legacy_cli, tmp_path):
...
def test_v2_full_document_validates(run_legacy_cli, tmp_path):
    ...
    exit_code, stdout, stderr = _run(run_legacy_cli, tmp_path, json.dumps(_V2_DOCUMENT))
    assert (exit_code, stderr) == (0, "")
    assert stdout == canonical_json({"valid": True})
...
def test_v3_non_increasing_points_are_refused(run_legacy_cli, tmp_path):
    document = {
        "schema_version": "1.0.0",
        "meta": {"name": "v3"},
        "attributes": {
...
def test_v4_tier_pattern_exact_set_matching(run_legacy_cli, tmp_path):
    document = {
        "schema_version": "1.0.0",
        "meta": {"name": "v4"},
        "attributes": {
...
def test_duplicate_accepts_on_attribute_is_a_structural_refusal(
    run_legacy_cli, tmp_path
):
...
def test_duplicate_accepts_on_tier_pattern_is_a_structural_refusal(
    run_legacy_cli, tmp_path
):
...
def test_probability_empty_bounds_is_refused(run_legacy_cli, tmp_path):
...
def test_probability_bounds_outside_domain_is_refused(run_legacy_cli, tmp_path):
...
def test_number_inverted_bounds_is_refused(run_legacy_cli, tmp_path):
...
def test_probability_direct_base_outside_domain_is_refused(run_legacy_cli, tmp_path):
...
def test_probability_bounds_narrowing_within_domain_is_valid(run_legacy_cli, tmp_path):
    ...
    exit_code, stdout, stderr = _run(run_legacy_cli, tmp_path, json.dumps(document))
    assert (exit_code, stderr) == (0, "")
    assert stdout == canonical_json({"valid": True})


def test_number_bounds_with_only_floor_is_valid(run_legacy_cli, tmp_path):
...
def test_probability_direct_base_at_domain_boundary_is_valid(
    run_legacy_cli, tmp_path, boundary
):
...
def test_v12_reserved_section_is_refused(run_legacy_cli, tmp_path):
    document = {
        "schema_version": "1.0.0",
        "meta": {"name": "reserved"},
        "builds": {},
...
def test_schema_reference_disagreement_is_refused(run_legacy_cli, tmp_path):
    content = (
        '{"schema_version": "1.0.0", "meta": {"name": "x"}, '
...
def test_schema_reference_agreement_validates(run_legacy_cli, tmp_path):
    content = json.dumps(
        {
            "schema_version": "1.0.0",
            "meta": {"name": "x"},
...
def test_explicit_null_schema_reference_is_a_structural_refusal(
    run_legacy_cli, tmp_path
):
...
def test_explicit_null_optional_field_is_a_structural_refusal(run_legacy_cli, tmp_path):
...
def test_explicit_null_reserved_section_is_still_reserved(run_legacy_cli, tmp_path):
...
def test_mutual_base_formula_cycle_refuses_both_bases(run_legacy_cli, tmp_path):
    document = {
        "schema_version": "1.0.0",
        "meta": {"name": "cycle"},
        "attributes": {
...
def test_max_depth_expression_tree_validates_in_linear_time(run_legacy_cli, tmp_path):
...
def test_deep_expression_tree_is_refused(run_legacy_cli, tmp_path):
...
def test_max_size_wide_tree_validates_in_linear_time(run_legacy_cli, tmp_path):
...
def test_wide_expression_tree_exceeding_node_cap_is_refused(run_legacy_cli, tmp_path):
...
def test_operator_arity_violation_is_a_structural_refusal(
    formula, run_legacy_cli, tmp_path
):
...
def test_v5_instant_effect_declaring_stacking_is_refused(run_legacy_cli, tmp_path):
...
def test_v5_timed_effect_without_stacking_is_refused(run_legacy_cli, tmp_path):
    document = _effects_document(
        {
            "items": {
                "buff": {
...
def test_v5_timed_all_one_shot_with_stacking_is_valid_but_inert(
    run_legacy_cli, tmp_path
):
...
def test_v6_all_one_shot_declaring_period_is_refused(run_legacy_cli, tmp_path):
    document = _effects_document(
        {
            "items": {
                "strike": {
...
def test_v6_continuous_only_with_period_is_valid(run_legacy_cli, tmp_path):
...
def test_v6_period_below_minimum_granularity_is_refused(run_legacy_cli, tmp_path):
    document = _effects_document(
        {
            "stacking_types": {"combine": {"aggregation": "stack"}},
            "items": {
...
def test_v6_granularity_and_tick_budget_report_all(run_legacy_cli, tmp_path):
...
def test_v6_continuous_only_tick_budget_is_refused(run_legacy_cli, tmp_path):
...
def test_magnitude_referencing_its_own_target_is_valid(run_legacy_cli, tmp_path):
...
def test_magnitude_referencing_undeclared_attribute_is_refused(
    run_legacy_cli, tmp_path
):
    document = _effects_document(
        {
...
def test_magnitude_lookup_table_empty_is_form_points_insufficient(
    run_legacy_cli, tmp_path
):
...
def test_magnitude_deep_expression_tree_is_refused(run_legacy_cli, tmp_path):
...
def test_magnitude_wide_expression_tree_is_refused(run_legacy_cli, tmp_path):
...
def test_magnitude_piecewise_non_increasing_points_is_refused(run_legacy_cli, tmp_path):
...
def test_valid_named_form_magnitude_still_validates(run_legacy_cli, tmp_path):
...
def test_nonexistent_path_is_a_usage_error(run_legacy_cli, tmp_path):
    exit_code, stdout, stderr = run_legacy_cli(
...
def test_directory_path_is_a_usage_error(run_legacy_cli, tmp_path):
