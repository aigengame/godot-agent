"""Independent Runtime consumer for the finite #878 priority witness.

Execution reads selected RIR semantics. The independently checked source context
is used only to admit that RIR and to select input/output wire contracts. This
module deliberately refuses scenarios outside the two maintained priority cases.
"""
...
_PRIORITY_EXPERIMENT_IDENTITIES = frozenset(
    {
        "sha256:e1c22043d7e4830038ed32a45ec78fa6093a96a89d187355c13343627506b775",
        "sha256:8800e7dbd221f335edb1b2ef3eb8d06bc96ac1b00571360b65a19a1f0c6775d1",
        "sha256:7383b7912882b7de3fead8527b778f1c3222f9815b2b3b0cde3b02f8aa3e1ec5",
        "sha256:60b1f583aed741e34e5a8fb8612396a867eabc8af2b40bae3b222215a28f43fa",
    }
)
...
def _selected_language_view(selected_semantics):
    """Project only the RIR-selected rows expected by the existing B executor."""
    packages = {}
    for row in selected_semantics["nominal_types"]:
        package = packages.setdefault(
            row["package"], {"id": row["package"], "semantic_closure": []}
        )
        package["semantic_closure"].append(
            {
                "authority_path": "language.nominal_types",
                "definitions": [deepcopy(row["definition"])],
            }
        )
    return {
        "language": {
            "packages": list(packages.values()),
            "constructors": deepcopy(selected_semantics["constructors"]),
            "structured_operations": [
                deepcopy(row["definition"])
...
def _admit_selected_nominal(selected_semantics, envelope):
    """Admit one finite-witness typed value from its selected constructors."""
    if not isinstance(envelope, dict) or set(envelope) != {"type", "value"}:
        raise ValueError("input nominal value is not a closed typed envelope")
    coordinate = envelope["type"]
    if not isinstance(coordinate, dict) or set(coordinate) != {"package", "id"}:
        raise ValueError("input nominal type coordinate is not closed")
    types = {(row["package"], row["id"]): row for row in selected_semantics["types"]}
    definitions = {
        (row["package"], row["definition"]["id"]): row["definition"]
        for row in selected_semantics["nominal_types"]
    }
    if (coordinate["package"], coordinate["id"]) not in definitions:
        raise ValueError("input root is not one of the four selected nominal types")
    constructors = {
        row["id"]: row["value_rule"] for row in selected_semantics["constructors"]
    }
    numeric = selected_semantics["execution_laws"]["runtime_program"]["numeric"]
    maximum_steps = selected_semantics["execution_resources"]["max_rule_match_steps"]
    steps = 0

    def admit(reference, value):
        nonlocal steps
        steps += 1
        if steps > maximum_steps:
            raise ValueError("input nominal admission exceeds selected rule budget")
        key = (reference["package"], reference["id"])
        declared = types.get(key)
        if declared is None:
            raise ValueError("input nominal type is not selected")
        rule = constructors[declared["constructor"]]
        if rule["operator"] == "exact-integer":
            if (
                not isinstance(value, int)
                or isinstance(value, bool)
                or not numeric["minimum"] <= value <= numeric["maximum"]
            ):
                raise ValueError("input quantity is outside selected numeric semantics")
            return
        definition = definitions[key]["definition"]
        if rule["operator"] == "enum-member":
            if value not in definition[rule["members_member"]]:
                raise ValueError("input enum member is not selected")
            return
        if rule["operator"] == "bounded-list":
            if (
                not isinstance(value, list)
                or len(value) > definition[rule["maximum_length_member"]]
            ):
                raise ValueError("input list exceeds selected bound")
            for member in value:
                admit(definition[rule["element_member"]], member)
            return
        if rule["operator"] == "closed-record":
            fields = {
                row[rule["field_name_member"]]: row[rule["field_type_member"]]
                for row in definition[rule["fields_member"]]
            }
            if not isinstance(value, dict) or set(value) != set(fields):
                raise ValueError("input record does not match selected fields")
            for name, member_type in fields.items():
                admit(member_type, value[name])
            return
        raise IndependentRuntimeUnsupported("unimplemented selected constructor")

    admit(coordinate, envelope["value"])
...
def _build_identity():
    sources = []
    for name in (
        "priority_runtime_independent_support.py",
...
def _supported_shape(specification, rir):
    if specification.get("id") not in {
        "example.priority-window.baseline",
        "example.priority-window.variant",
    }:
        raise IndependentRuntimeUnsupported("only the priority witness is supported")
    if (
        _reference_content_identity("priority-experiment-input-v1", specification)
        not in _PRIORITY_EXPERIMENT_IDENTITIES
    ):
        raise IndependentRuntimeUnsupported(
            "only the four fixed priority Experiment inputs are supported"
        )
    ...
    if scenario.get("id") != "priority":
        raise IndependentRuntimeUnsupported("only the priority Scenario is supported")
    ...
    if scenario["terminal_condition"] != {"kind": "queue-drained"}:
    ...
    actual_operators = {operators.get(node) for node in required["instruction_nodes"]}
    if actual_operators != _PRIORITY_OPERATORS:
        raise IndependentRuntimeUnsupported(
            "priority instruction semantics changed: "
            + repr(sorted(str(operator) for operator in actual_operators))
        )
    available = {
        "operation_kinds": ["event-program", "pure-expression"],
        "instruction_nodes": sorted(required["instruction_nodes"]),
        "effects": [
            "event.commit",
            "event.schedule",
            "metric.observe",
...
def reference_runtime_artifacts(
    ...
    def admit_value(key, value):
        ...
        if declaration.get("value_kind") == "nominal-structured":
            ...
            _admit_selected_nominal(semantic, value)
        else:
    ...
    while pending:
        ...
        if active["kind"] == "external-input":
            ...
            frame = ReferenceEventFrame(updated, frame.node_steps)
            ...
            actual = reference_execute_event(
                {"meta_format": {"runtime_program": runtime}},
                ...
                language_bundle=_selected_language_view(semantic),
...
def reference_admits_runtime_artifacts(
    context, rir, specification, artifacts, *, expected
):
    """Admit supplied members from one already-produced fixed Build B result."""
    try:
        _supported_shape(specification, rir)
