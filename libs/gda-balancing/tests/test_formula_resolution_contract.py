...
from test_resolution_parse_reason import _profile as _authored_profile
...
def test_authority_refuses_incoherent_formula_resolution(mutation):
    ...
    profile = _authored_profile(authored)
...
def test_authority_refuses_unsupported_kernel_formula_resolution(mutation, monkeypatch):
    ...
    # A missing Formula owner prevents the earlier derived RIR projection.
    subject = (
        "language.definitions"
        if mutation == "missing"
        else "kernel.meta-format.formula-resolution"
    )
    for consumer in (_consumer_a, _consumer_b):
        result = consumer(kernel, graph)
        assert not result["admitted"]
...
def test_public_model_consumes_formula_source_member_selector(tmp_path):
    ...
    assert (
        ...
        not in artifacts["package-lock"]["resolution_profile"]["formula_resolution"]
    )
    assert module_schema["properties"]["calculations"]["semantic_member"] == "formulas"
