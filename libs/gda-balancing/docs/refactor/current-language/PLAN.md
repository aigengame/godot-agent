...
The machine baseline has five constructors (Quantity, Enum, List, Record, Ref), with Quantity currently exact-int64 scalar unit 1. The architecture's fifteen prospective constructors are an envelope, not present support. Add a constructor only when a mapped scenario cannot be expressed faithfully using current constructors and content. Keep #512 continuous-equation/solver policy explicitly open; do not sneak floating-point or unbounded integration in under arithmetic cleanup.
...
| Fifteen-constructor wish list, generalized interfaces/caches | Defer implementation | Preserve requirement ownership and explicit gaps; add only when a mapped witness needs them. |
