...
| S6b early extension falsifier | Run one finite non-RPG priority source→RIR→run witness against the fixed Kernel and unchanged host dispatch; freeze independent builds A/B before deriving the selected-dependency binding | Original and explicitly renamed authorities each produce eight Model and six Runtime members; A/B mutually admit the exact members; the binding covers complete admitted `selected_semantics`, RIR identity, all eight Model identities and only the four Type plus eight Operation coordinates deliberately renamed. Direct negatives refuse missing/stale bindings and illegal nesting/callback/phase/depth | If it needs a new primitive, genre switch, host callback or fixed scheduler phase, reopen the basis. Whole-LDB inventory, arbitrary rewriting and a general reference platform are forbidden; full breadth remains S8/#575. |
...
S6b must demonstrate input-dependent behavior through admitted reusable priority-window Operations, not a Model or fixture that prescribes the expected trace. Under the same rules and fixed builds, a baseline sequence and a discriminating responder-order, counter-target or pass/close variant must produce the specified per-input-boundary stack, priority and stable pending identities, followed by the expected different final outcome or typed refusal. Kernel laws, constructors, phases and compiler/evaluator dispatch remain unchanged. The selected-dependency binding is derived from the produced Model sets, then verified fail-closed; it is not a second hand-maintained semantic taxonomy. [#878](https://github.com/aigengame/godot-agent/issues/878) owns this acceptance criterion and [the bounded delivery record](PRIORITY-WINDOW.md) owns the exact implementation and rollback boundary.

The minimal S6b extension tracer runs before broad S7 work. It exercises independently fixed builds, one explicit twelve-coordinate rename and mutual artifacts for the narrow witness. It must not scan or rewrite unrelated LDB/vector/artifact/string surfaces. Functional results remain candidate/open; authenticated Extension Invariance or other formal claims do not close until their existing application/trust activation conditions are met. S8 retains the complete #575 scenario family and final-graph invariance obligations.
