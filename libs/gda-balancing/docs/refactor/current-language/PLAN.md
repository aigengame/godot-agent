...
| S6b early extension falsifier | Promote a minimal non-RPG nested-priority Source→RIR→run witness against the candidate fixed Kernel and unchanged host dispatch; fix both independent builds, derive the declared witness's selected execution closure, enumerate and rename only twelve explicitly selected extension-owned Type/Operation coordinates, and exchange the actual generated/consumed artifacts and results | Reusable priority-window Operations execute the proposal/counter-to-counter/pass-or-cancel/final-resolution path plus a discriminating input variant with specified per-boundary state/outcomes; the complete admitted `selected_semantics` canonical hash/RIR semantic identity and all eight Model artifact content identities bind the selected meaning, mismatch/omission cases refuse, and A/B mutually consume all eight Model plus six Runtime artifacts/results | If it needs a genre switch/new fixed scheduler phase, reopen the basis now. #878 has no hand-authored dependency-category manifest; unrelated packages, vector sets, artifact families and ordinary data strings are outside it. #575 retains the exhaustive bounded bijection and formal Receipt; full breadth remains S8/#575. |
...
| S11 full closure and v1 decision | All mapped requirements/gates, package CI and real consumer validation; current docs and release artifacts; #879 physically deletes RIR `domain_kind`, the obsolete `maximum` RIR wire-union branch, and residual fallback/compatibility bindings | Acceptance is evidence-backed; deletion scans and public consumers prove the obsolete mechanisms are absent; operational budget, recovery, security, install and rollback gates pass. Explicit owner chooses v1.0 public contract | Not automatically achieved by dependency closure, deprecation, ignored fields, partial deletion, green unit tests or an internal tag. |
...
The minimal S6b extension tracer runs before broad S7 work. It exercises independently fixed builds,
the priority witness's selected execution closure, enumeration and renaming of only twelve selected
extension-owned Type/Operation coordinates, binding of the complete admitted `selected_semantics`
canonical hash/RIR semantic identity and all eight Model artifact content identities, mismatch/
omission refusals, and mutual consumption of the actual eight Model and six Runtime artifacts/results.
It has no hand-authored dependency-category manifest and does not inventory unrelated packages, unselected vector sets or
artifact families, or ordinary data strings. A 2026-09-21 backtrace found that extending #878 into a whole-LDB, all-vector,
arbitrary-string inventory created an NFR platform rather than acceptance progress; that generic
inventory machinery is deleted. Functional results remain candidate/open; #575 retains the formal
Extension Invariance Receipt, exhaustive bounded bijection and complete scenario-family obligations,
and #542–#544 retain authenticated claim activation.
