...
| Tracer `RPG-CHECK-01` | Resolve threshold or opposed hit checks with named randomness, explicit tie policy, and success degree. | `game.check.typed-resolution`; `game.check.resolve@1` | check owns resolution; combat consumes the typed outcome | `rpg.target-check-v1` | `rpg.check.tie-policy-missing-refused-v1` | per-draw stream identity and draw index in the Event trace, `hit_degree` Metric sample |
...
state write. A cardinality below one is a configuration refusal, not exhaustion. A check invoked
without a declared tie policy produces the `tie-policy-missing` refusal before any named-stream
draw, whether or not a tie would have occurred; a zero margin under a declared policy is that
policy's verdict, not a refusal. The `rpg.target.*` and `rpg.check.*` vector ids above remain
reserved for the RPG template release (#566); the #547 slice binds package-owned vectors and one
maintained Golden example, which supply evidence toward the rows without closing them.
