...
|---|---|---|---|---|---|---|
| Tracer `RPG-STAT-01` | Compose explicitly typed base/parameter/state symbols into derived combat values through progression, build and effect contributions with declared kind, unit, rounding and cap laws. | `core.typed-symbols-v1`; `core.expression.derive@1`; `quantity.add`; `quantity.multiply`; `quantity.floor-divide`; `quantity.minimum`; `game.build.contribution@1`; `game.progression.contribution@1`; `game.effect.contribute@1`; `game.combat.damage-v1` | Model Source owns named Formula declarations and exact Operation-slot bindings; core/LDB owns pure-expression typing/evaluation/context/closure laws; progression, build and effect packages own their respective contribution sources and Formula-slot contracts; combat consumes the final derived Quantity and owns damage application | `rpg.stat.composition-v1` | `rpg.stat.dependency-cycle-refused-v1`; `rpg.stat.kind-mismatch-refused-v1`; `rpg.stat.unit-mismatch-refused-v1`; `rpg.stat.round-down-boundary-v1`; `rpg.stat.cap-exact-boundary-v1`; `rpg.stat.cap-clamped-boundary-v1` | resolved dependency graph, exact Formula/evaluation-site/context and refusal/resource-closure identities, Model explanation, contribution provenance, derived contribution Metric samples, Numeric profile, final typed Quantity, applied damage and terminal target HP |
| Tracer `RPG-TARGET-01` | Resolve targets from the current dynamic entity set with declared filtering, cardinality, stable order, tie and empty behavior. | `game.entity.dynamic-sets`; `game.query.dynamic-targets`; `game.query.select@1`; Kernel `where-equal`, `order-by`, `take`, `count` (bADR-0028) | entity owns identity and membership types; query owns the typed candidate view and selection; combat consumes the resolved targets | `rpg.target-check-v1` | `rpg.target.empty-outcome-v1`; `rpg.target.empty-policy-missing-refused-v1` | resolved candidate order, `target_count` Metric sample |
...
| Tracer `RPG-CHECK-01` | Resolve threshold or opposed hit checks with named randomness, explicit tie policy, and success degree. | `game.check.typed-resolution`; `game.check.resolve@1` | check owns resolution; combat consumes the typed outcome | `rpg.target-check-v1` | `rpg.check.tie-policy-missing-refused-v1`; `rpg.combat.profile-ambiguous-refused-v1` | per-draw stream identity and draw index in the Event trace, `hit_degree` Metric sample |
...
| Roguelike `ROGUE-REWARD-01` | Draw seeded constrained rewards under one closed fixed-weight, stateful pity, or guarantee rarity-policy variant, with an independent no-reward fallback for an empty eligible pool, without interpreting a pity cap as a guarantee. | `game.generation.constrained-reward-v1`; `game.generation.rarity-policy-v1`; `game.generation.draw@1` | generation owns the eligible pool, draw consumption, rarity-policy state, exhaustion fallback, and disposition; destination packages consume only a committed reward disposition | `roguelike.reward-build-v1` | `roguelike.reward.pity-cap-boundary-v1`; `roguelike.reward.selection-exhausted-refused-v1`; `roguelike.reward.exhaustion-fallback-outcome-v1`; `roguelike.reward.guarantee-policy-unsatisfiable-refused-v1`; manifest-bound `generation.select.empty-refusal`; manifest-bound `generation.select.no-reward-outcome` | stream/candidate/draw indexes, ordered eligible pool, selected policy variant, declared no-reward fallback, reward score, rarity state before/after, guarantee state, exhaustion decision, and fallback disposition |
...
| Roguelike `ROGUE-BUILD-01` | Apply run-time build conflicts, synergies and atomic replacement to generated rewards, route the accepted/rejected result through its declared disposition, and complete a no-reward disposition before plan lookup without changing build state. | `game.build.constraints-v1`; `game.build.atomic-replacement-v1`; `game.build.select@1`; `game.build.replace@1`; `game.generation.draw@1` | generation proposes; build admits/rejects/replaces; the declared destination package handles disposition without a mandatory economy step | `roguelike.reward-build-v1` | `roguelike.build.conflict-outcome-v1`; `roguelike.build.atomic-replacement-boundary-v1`; manifest-bound `build.replace.no-reward-outcome` | conflict ids, synergy changes, old/new selection and slot identities, accepted/rejected reward disposition, and for no reward: no plan lookup, a `not-produced` result, zero RNG draws, and unchanged build state |
...
- `rpg.stat.composition-v1`: base and tunable typed symbols combine progression, equipment and
  effect contributions into a derived combat Quantity. Named Formulas expose each contribution as
  a derived Symbol. The Experiment observes them through committed-Snapshot Metrics. Effect
  percentage damage rounds down before the final sum is capped. `game.combat.damage-v1` consumes the
  final Quantity, and the Experiment observes applied damage and terminal target HP.
...
- `rpg.target-check-v1`: an authored candidate set of allies and enemies with declared life states,
  ordering keys, and tie keys feeds `game.combat.target-check@1`. The declared policy filters by
  faction and life state, orders by the ordering key in the declared direction, breaks equal keys by
  the tie key and then by input order, and takes the declared cardinality. The first resolved
  target's combat profile supplies the defense that a threshold or opposed named-stream check
  resolves against under an explicit tie policy. The Experiment observes the resolved order,
  `target_count`, and `hit_degree`, and reaches the `no-target` outcome when every enemy is
  defeated. The scenario applies no damage and spends no resource; `rpg.combat.cast-v1` consumes
  this path once #548 admits Action plans.
...
- `roguelike.reward-build-v1`: fixed, pity, and guarantee rarity-policy vectors compose with a
  separately declared exhaustion fallback, run-local build conflicts, synergies, replacement, and
  typed destination dispositions.
...
Selection exhaustion occurs only when the already ordered eligible `RewardOption` list is empty.
With one declared `no_reward_on_empty` value, it produces the `no-reward` gameplay outcome. Without
that value, it produces the `selection-exhausted` refusal. The fallback is independent of the fixed,
pity, or guarantee policy variant. A nonempty pool that cannot satisfy its guarantee policy produces
the distinct `guarantee-policy-unsatisfiable` refusal. Contradictory option or fallback data is also
a configuration refusal, not selection exhaustion. Currency and relaxed-pool behavior are not part
of this contract.

For `ROGUE-BUILD-01`, the current structured reward vertical slice binds the downstream no-reward
branch to the manifest-bound `build.replace.no-reward-outcome` Operation vector and the public
`roguelike.reward-build-v1` path. They prove that `game.build` completes before plan lookup, produces
no result or RNG draw, and leaves build state unchanged. This evidence closes only the no-reward
branch; the row remains open until all other named requirements and evidence are complete.

Target selection exhaustion occurs only when the filtered candidate list is empty. With one declared
`on_empty` value, it produces the `no-target` gameplay outcome and commits an empty resolution with
`target_count` zero. Without that value, it produces the `empty-policy-missing` refusal before any
state write. A cardinality below one is a configuration refusal, not exhaustion. A selected target
with no combat profile row produces the `combat-profile-missing` refusal, and one with more than one
profile row produces the `combat-profile-ambiguous` refusal; both occur before any named-stream
draw. A check invoked without a declared tie policy produces the `tie-policy-missing` refusal before
any named-stream draw, whether or not a tie would have occurred; a zero margin under a declared
policy is that policy's gameplay outcome, not a refusal. The `rpg.target.*`, `rpg.check.*`, and
`rpg.combat.*` vector ids above remain reserved for the RPG template release (#566); the #547 slice
binds package-owned vectors and one maintained Golden example, which supply evidence toward the
rows without closing them.
