...
actor-health input and proves no gameplay-resource, RNG, or state change on ineligibility. The
public local-service tracer stops consecutive complete Experiment revisions on the explicit defeat
outcome. This slice does not define target eligibility or distinguish a new threshold crossing from
an already-satisfied target condition. The evidence does not close the row. Ordered typed damage
components, matching defenses, shield ordering, entity-owned defeat storage, and the row's reserved
Golden scenario remain open.
...
|---|---|---|---|---|---|---|
| Tracer `RPG-STAT-01` | Compose explicitly typed base/parameter/state symbols into derived combat values through progression, build and effect contributions with declared kind, unit, rounding and cap laws. | `core.typed-symbols-v1`; `core.expression.derive@1`; `game.build.contribution@1`; `game.progression.contribution@1`; `game.effect.contribute@1` | Model Source owns named Formula declarations and exact Operation-slot bindings; core/LDB owns pure-expression typing/evaluation/context/closure laws; packages own independently composable contribution sources and Formula-slot contracts | `rpg.stat.composition-v1` | `rpg.stat.dependency-cycle-refused-v1`; `rpg.stat.kind-unit-mismatch-refused-v1`; `rpg.stat.round-cap-boundary-v1` | resolved dependency graph, exact Formula/evaluation-site/context and refusal/resource-closure identities, Model explanation, contribution provenance, Numeric profile and final typed Quantity |
