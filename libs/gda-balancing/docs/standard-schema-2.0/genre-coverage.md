...
| Roguelike `ROGUE-REWARD-01` | Draw seeded constrained rewards under one closed fixed-weight, stateful pity, or guarantee rarity-policy variant, with exhaustion fallback declared in a separate field, without interpreting a pity cap as a guarantee. | `game.generation.constrained-reward-v1`; `game.generation.rarity-policy-v1`; `game.generation.draw@1` | generation owns eligible pool, draw consumption, rarity-policy state, exhaustion fallback and disposition; destination packages consume only a produced reward disposition | `roguelike.reward-build-v1` | `roguelike.reward.pity-cap-boundary-v1`; `roguelike.reward.exhaustion-fallback-outcome-v1`; `roguelike.reward.guarantee-policy-unsatisfiable-refused-v1` | stream/candidate/draw indexes, ordered eligible pool, selected policy variant, declared exhaustion fallback, rarity state before/after, guarantee state and fallback disposition |
...
Selection exhaustion is an outcome only when the authored reward pool declares an applicable
fallback such as no reward, currency, or a relaxed pool. The fallback is independent of the primary
fixed, pity, or guarantee policy variant. A policy whose admitted constraints cannot select any
result and declares no fallback produces the declared exhaustion refusal; implementations may not
choose between those meanings implicitly.
