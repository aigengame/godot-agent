...
|---|---|---|---|---|---|---|
| Tracer `RPG-STAT-01` | Compose explicitly typed base/parameter/state symbols into derived combat values through progression, build and effect contributions with declared kind, unit, rounding and cap laws. | `core.typed-symbols-v1`; `core.expression.derive@1`; `quantity.add`; `quantity.multiply`; `quantity.floor-divide`; `quantity.minimum`; `game.build.contribution@1`; `game.progression.contribution@1`; `game.effect.contribute@1` | Model Source owns named Formula declarations and exact Operation-slot bindings; core/LDB owns pure-expression typing/evaluation/context/closure laws; progression, build and effect packages own their respective contribution sources and Formula-slot contracts | `rpg.stat.composition-v1` | `rpg.stat.dependency-cycle-refused-v1`; `rpg.stat.kind-mismatch-refused-v1`; `rpg.stat.unit-mismatch-refused-v1`; `rpg.stat.round-down-boundary-v1`; `rpg.stat.cap-exact-boundary-v1`; `rpg.stat.cap-clamped-boundary-v1` | resolved dependency graph, exact Formula/evaluation-site/context and refusal/resource-closure identities, Model explanation, contribution provenance, derived contribution Metric samples, Numeric profile and final typed Quantity |
...
- `rpg.stat.composition-v1`: base and tunable typed symbols combine progression, equipment and
  ...
  percentage damage rounds down before the final sum is capped.
