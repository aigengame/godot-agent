...
| `release-session` | Input: session handle. Result: released session handle or `unknown_execution_session`. | Path `session_id` and an empty body project to `ReleaseExecutionSessionRequest`, then to `ExecutionSessionDeletedResponse` or the shared OHS error | `DELETE /v1/execution-sessions/{session_id}` |
