...
- a run GDA ENDED — at the caller's ``--timeout``, or early because a declared
  ...
  ``tests/runtime/test_launch.py``; what is asserted here is the classification.

They are the recipe's own test surface, complementary to the e2e round-trip in
``tests/script/test_e2e_script_run.py`` (real Godot).
"""
...
# is no defect — see tests/script/test_script_error_parser.py for the parser-level
...
def test_a_whitespace_entry_path_is_a_failure_despite_the_zero_exit():
    ...
    # tests/script/test_script_error_parser.py).
    outcome, _ = _run(
...
# that shape is asserted against real pipes in ``tests/runtime/test_launch.py``.
...
def test_a_script_run_failure_decided_before_any_launch_carries_no_evidence():
    ...
    # this guard lives in tests/cli/test_error_registry.py.)
    outcome, _ = _run(
